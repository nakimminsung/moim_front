...
function SellerLogin() {
  ...
  const onSubmit = () => {
    ...
    let url = "http://localhost:9000/member/Sellercheck"
    ...
    axios.post(url, { email, password })
      .then(res => {
        ...
        if (res.data.check == 1) {
          ...
          // 일단 Home으로 이동
          navi("/");
          window.location.reload(); // 새로고침 (refresh를 해줘야한다.)
        } else {
    ...
    <>
      <LoginWrap>
        <LoginContainer>
          ...
          <LoginSignupContent>
            <VerticalButtons>
              <BarButton className="email" href="/sellerJoin">
                이메일로 가입하기
              </BarButton>
            </VerticalButtons>
          </LoginSignupContent>
          ...
          <MoveSignUP>
            <span>호스트 계정이 없으신가요? <a href='/sellerJoin'>호스트 회원가입</a></span>
          </MoveSignUP>
        </LoginContainer>
      </LoginWrap>
    </>
